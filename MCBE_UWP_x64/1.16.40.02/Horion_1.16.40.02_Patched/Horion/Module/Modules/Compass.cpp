...
Compass::Compass() : IModule(0, Category::VISUAL, "Compass") {
	...
	registerBoolSetting("Use X/Z labels", &useXZ, useXZ);
}
...
void Compass::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	for (int off = -range; off <= range; off++) {
		...
		switch (oDeg) {
		case 0:
			drawCenteredText(vec2_t(xOff, 30.f), useXZ ? "Z-" : "N", 1, majorOpacity);
			break;
		case 45:
			drawCenteredText(vec2_t(xOff, 30.f), useXZ ? "X+Z-" : "NE", 1, majorOpacity);
			break;
		case 90:
			drawCenteredText(vec2_t(xOff, 30.f), useXZ ? "X+" : "E", 1, majorOpacity);
			break;
		case 135:
			drawCenteredText(vec2_t(xOff, 30.f), useXZ ? "X+Z+" : "SE", 1, majorOpacity);
			break;
		case 180:
			drawCenteredText(vec2_t(xOff, 30.f), useXZ ? "Z+" : "S", 1, majorOpacity);
			break;
		case 225:
			drawCenteredText(vec2_t(xOff, 30.f), useXZ ? "X-Z+" : "SW", 1, majorOpacity);
			break;
		case 270:
			drawCenteredText(vec2_t(xOff, 30.f), useXZ ? "X-" : "W", 1, majorOpacity);
			break;
		case 315:
			drawCenteredText(vec2_t(xOff, 30.f), useXZ ? "X-Z-" : "NW", 1, majorOpacity);
			break;
		}
