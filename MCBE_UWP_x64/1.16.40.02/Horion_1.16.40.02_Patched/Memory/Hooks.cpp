...
void Hooks::ClickFunc(__int64 a1, char mouseButton, char isDown, __int16 mouseX, __int16 mouseY, __int16 relativeMovementX, __int16 relativeMovementY, char a8) {
	...
	static auto clickGuiModule = moduleMgr->getModule<ClickGuiMod>();
	static auto autoCords = moduleMgr->getModule<AutoCordsTest>();
	...
	// When AutoCords is enabled in axe tool mode, prevent right-click from opening block interfaces
	// while holding a wooden axe (so you don't have to press ESC).
	if (autoCords != nullptr && autoCords->isEnabled() && autoCords->shouldAxeToolMode()) {
		if (isDown && GameData::isRightClickDown()) {
			...
			if (lp != nullptr) {
				...
				if (supplies != nullptr && supplies->inventory != nullptr) {
					...
					if (stack != nullptr && stack->item != nullptr && *stack->item != nullptr && (*stack->item)->itemId == 271) {
						return;
					}
				}
			}
